...
are not part of the quartet involved in the torsion [@doi:10.1186/s13321-019-0371-6]. Figure 2A illustrates
...
WBOs are conformationally dependent [@doi:10.1039/c6ra17055b] so we investigated this dependence to understand if WBOs
...
in a system. However, it can become computationally expensive to calculate the WBO for all conformations and if we want to use
WBOs as a surrogate to determine is our fragment is representative of the parent molecule, we need a way to capture informative
conformation-independent WBOs. Electronically Least-interacting Functional groups (ELF) conformation selection implemented in quacpac
...
WBOs, we calculated ELF10 WBOs for the kinase inhibitor set shown in supplementary figure 1.
