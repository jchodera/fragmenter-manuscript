...
{`JDC: Detailed Methods go after the Conclusion.`}{.red}
...
In the future, `fragmenter` will be incorporated into the `openforcefield` toolkit and will have the option to use RDKit [@url:http://www.rdkit.org/], an open source cheminformatic libraries.
 However, given that RDKit supports EHT instead of AM1 {`JDC: As of which version?`}{.red}, the fragments might be different.
